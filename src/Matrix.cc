...
void Matrix::Init(Local<Object> target) {
  ...
  Nan::SetPrototypeMethod(ctor, "getPerspectiveTransform",
...
NAN_METHOD(Matrix::New) {
  ...
  if (info.Length() == 0) {
    ...
    mat = new Matrix(info[0]->IntegerValue(), info[1]->IntegerValue(),
                     info[2]->IntegerValue(), Buffer::Data(info[3]),
...
Matrix::Matrix() : node_opencv::Matrix() { mat = cv::Mat(); }

Matrix::Matrix(int rows, int cols) : node_opencv::Matrix() {
  mat = cv::Mat(rows, cols, CV_32FC3);
}

Matrix::Matrix(int rows, int cols, int type) : node_opencv::Matrix() {
  // std::cout << "Matrix " << rows << std::endl;
  mat = cv::Mat(rows, cols, type);
}

Matrix::Matrix(cv::Mat m, cv::Rect roi) : node_opencv::Matrix() {
  mat = cv::Mat(m, roi);
}
...
NAN_METHOD(Matrix::Set) {
  ...
  if (info.Length() == 4) {
    ...
    case CV_32FC3:
      ...
      // printf("!!!i %x, %x, %x", (vint >> 16) & 0xff, (vint >> 8) & 0xff,
      // (vint) & 0xff);
      break;
    default:
...
NAN_METHOD(Matrix::GetData) {
  ...
  int size = self->mat.rows * self->mat.cols * self->mat.channels();
  ...
  if (info.Length() == 2) {
    ...
    if (info.Length() == 1) {
      ...
      info.GetReturnValue().Set(
          Nan::New("Insufficient or wrong arguments").ToLocalChecked());
    }
  }
...
int getNormType(int type) {
  if ((type != cv::NORM_MINMAX) && (type != cv::NORM_INF) &&
    ...
    Nan::ThrowTypeError(
        "type value must be NORM_INF=1, NORM_L1=2, NORM_L2=4,"
...
NAN_METHOD(Matrix::Norm) {
  ...
  if (!info[infoCount]->IsUndefined() && info[infoCount]->IsObject()) {
    Matrix *mmask =
        Nan::ObjectWrap::Unwrap<Matrix>(info[infoCount]->ToObject());
    mask = mmask->mat;
    infoCount++;
  }
...
NAN_METHOD(Matrix::ToBuffer) {
  ...
  // SergeMv changes
  // img.toBuffer({ext: ".png", pngCompression: 9}); // default png compression
  // is 3
  ...
  if ((info.Length() > 0) && (info[0]->IsObject())) {
    ...
    if (options->Has(Nan::New<String>("jpegQuality").ToLocalChecked())) {
      int compression =
          options->Get(Nan::New<String>("jpegQuality").ToLocalChecked())
              ->IntegerValue();
    ...
    if (options->Has(Nan::New<String>("pngCompression").ToLocalChecked())) {
      int compression =
          options->Get(Nan::New<String>("pngCompression").ToLocalChecked())
              ->IntegerValue();
...
NAN_METHOD(Matrix::Ellipse) {
  ...
  if (info[0]->IsObject()) {
    ...
    if (options->Has(Nan::New<String>("axes").ToLocalChecked())) {
      Local<Object> axes =
          options->Get(Nan::New<String>("axes").ToLocalChecked())->ToObject();
      width =
          axes->Get(Nan::New<String>("width").ToLocalChecked())->Uint32Value();
      height =
          axes->Get(Nan::New<String>("height").ToLocalChecked())->Uint32Value();
    }
    if (options->Has(Nan::New<String>("thickness").ToLocalChecked())) {
      thickness = options->Get(Nan::New<String>("thickness").ToLocalChecked())
                      ->Uint32Value();
    }
    if (options->Has(Nan::New<String>("angle").ToLocalChecked())) {
      angle = options->Get(Nan::New<String>("angle").ToLocalChecked())
                  ->NumberValue();
    }
    if (options->Has(Nan::New<String>("startAngle").ToLocalChecked())) {
      startAngle = options->Get(Nan::New<String>("startAngle").ToLocalChecked())
                       ->NumberValue();
    }
    if (options->Has(Nan::New<String>("endAngle").ToLocalChecked())) {
      endAngle = options->Get(Nan::New<String>("endAngle").ToLocalChecked())
                     ->NumberValue();
    }
    if (options->Has(Nan::New<String>("lineType").ToLocalChecked())) {
      lineType = options->Get(Nan::New<String>("lineType").ToLocalChecked())
                     ->Uint32Value();
    }
    if (options->Has(Nan::New<String>("shift").ToLocalChecked())) {
      shift = options->Get(Nan::New<String>("shift").ToLocalChecked())
                  ->Uint32Value();
    }
...
NAN_METHOD(Matrix::FillPoly) {
  ...
  if (info[0]->IsArray()) {
    ...
    for (unsigned int i = 0; i < polyArray->Length(); i++) {
      Local<Array> singlePoly =
          Local<Array>::Cast(polyArray->Get(i)->ToObject());
...
public:
  ...
  // should go on `this`.
  void Execute() { res = cv::imwrite(this->filename, this->matrix->mat); }
  ...
  void HandleOKCallback() {
    ...
    Local<Value> argv[] = {Nan::Null(), Nan::New<Number>(res)};
...
NAN_METHOD(Matrix::BilateralFilter) {
  ...
  cv::bilateralFilter(self->mat, filtered, d, sigmaColor, sigmaSpace,
                      borderType);
...
NAN_METHOD(Matrix::Sobel) {
  ...
  if (info.Length() < 3)
    Nan::ThrowError("Need more arguments: sobel(ddepth, xorder, yorder, "
                    "ksize=3, scale=1.0, delta=0.0, "
                    "borderType=CV_BORDER_DEFAULT)");
  ...
  int ksize = 3;
  if (info.Length() > 3)
    ksize = info[3]->IntegerValue();
  double scale = 1;
  if (info.Length() > 4)
    scale = info[4]->NumberValue();
  double delta = 0;
  if (info.Length() > 5)
    delta = info[5]->NumberValue();
  int borderType = cv::BORDER_DEFAULT;
  if (info.Length() > 6)
    borderType = info[6]->IntegerValue();
  ...
  cv::Sobel(self->mat, result->mat, ddepth, xorder, yorder, ksize, scale, delta,
            borderType);
...
NAN_METHOD(Matrix::Flip) {
  ...
  if (info.Length() < 1 || !info[0]->IsInt32()) {
    Nan::ThrowTypeError(
        "Flip requires an integer flipCode argument "
...
NAN_METHOD(Matrix::ROI) {
  ...
  // Although it's an image to return, it is in fact a pointer to ROI of parent
  // matrix
  Local<Object> img_to_return =
      Nan::New(Matrix::constructor)->GetFunction()->NewInstance();
  Matrix *img = Nan::ObjectWrap::Unwrap<Matrix>(img_to_return);
...
NAN_METHOD(Matrix::Ptr) {
  ...
  Local<Object> return_buffer =
      Nan::NewBuffer((char *)data, self->mat.step).ToLocalChecked();
...
NAN_METHOD(Matrix::Abs) {
  Nan::HandleScope scope;
  Matrix *self = Nan::ObjectWrap::Unwrap<Matrix>(info.This());
  self->mat = cv::abs(self->mat);
  info.GetReturnValue().Set(Nan::Null());
}
...
NAN_METHOD(Matrix::Multiply) {
  ...
  cv::multiply(src1->mat, src2->mat, self->mat, scale);
...
NAN_METHOD(Matrix::FindContours) {
  ...
  if (info.Length() > 0) {
    if (info[0]->IsNumber())
      mode = info[0]->IntegerValue();
  }

  if (info.Length() > 1) {
    if (info[1]->IsNumber())
      chain = info[1]->IntegerValue();
  }

  Matrix *self = Nan::ObjectWrap::Unwrap<Matrix>(info.This());
  Local<Object> conts_to_return =
      Nan::New(Contour::constructor)->GetFunction()->NewInstance();
  Contour *contours = Nan::ObjectWrap::Unwrap<Contour>(conts_to_return);

  cv::findContours(self->mat, contours->contours, contours->hierarchy, mode,
                   chain);
...
NAN_METHOD(Matrix::HoughCircles) {
  ...
  double accumulatorThreshold =
      info.Length() < 4 ? 100 : info[3]->NumberValue();
...
NAN_METHOD(Matrix::Resize) {
   ...
   */
  int interpolation =
      (info.Length() < 3) ? (int)cv::INTER_LINEAR : info[2]->Uint32Value();
...
NAN_METHOD(Matrix::Rotate) {
  ...
  //-------------
  // If you provide only the angle argument and the angle is multiple of 90,
  // then
  // we do a fast thing
  bool rightOrStraight =
      (ceil(angle) == angle) && (!((int)angle % 90)) && (info.Length() == 1);
  if (rightOrStraight) {
...
NAN_METHOD(Matrix::WarpAffine) {
  ...
  int dstRows =
      info[1]->IsUndefined() ? self->mat.rows : info[1]->Uint32Value();
  int dstCols =
      info[2]->IsUndefined() ? self->mat.cols : info[2]->Uint32Value();
...
NAN_METHOD(Matrix::ConvertTo) {
  ...
  std::cout << "convertTo " << rtype << " " << CV_8U << " " << alpha << " "
            << beta << std::endl;
...
NAN_METHOD(Matrix::CvtColor) {
  ...
  if (info[1]->IsString()) {
    ...
    } else if (!strcmp(sTransform, "BayerBG2RGB")) {
      iTransform = CV_BayerBG2RGB;
    } else if (!strcmp(sTransform, "BayerGB2RGB")) {
      iTransform = CV_BayerGB2RGB;
    } else if (!strcmp(sTransform, "BayerRG2RGB")) {
      iTransform = CV_BayerRG2RGB;
    } else if (!strcmp(sTransform, "BayerGR2RGB")) {
      iTransform = CV_BayerGR2RGB;
...
NAN_METHOD(Matrix::Merge) {
  ...
  for (unsigned int i = 0; i < L; i++) {
    Matrix *matObject =
        Nan::ObjectWrap::Unwrap<Matrix>(jsChannels->Get(i)->ToObject());
...
NAN_METHOD(Matrix::FloodFill) {
  ...
  // int cv::floodFill(cv::InputOutputArray, cv::Point, cv::Scalar, cv::Rect*,
  // cv::Scalar, cv::Scalar, int)
  ...
  int ret = cv::floodFill(
      self->mat,
      setPoint(
          obj->Get(Nan::New<String>("seedPoint").ToLocalChecked())->ToObject()),
      setColor(
          obj->Get(Nan::New<String>("newColor").ToLocalChecked())->ToObject()),
      obj->Get(Nan::New<String>("rect").ToLocalChecked())->IsUndefined()
          ...
          : setRect(
                ...
                rect),
      setColor(
          obj->Get(Nan::New<String>("loDiff").ToLocalChecked())->ToObject()),
      setColor(
          obj->Get(Nan::New<String>("upDiff").ToLocalChecked())->ToObject()),
      4);
...
// Usage: output = input.templateMatches(min_probability, max_probability,
// limit, ascending, min_x_distance, min_y_distance);
  ...
  for (; (limit == 0 || index < limit) && it != end; ++it) {
    ...
    if (min_x_distance != 0 || min_y_distance != 0) {
      ...
      cv::Point bottom_right =
          cv::Point(std::min(max_x, pt.x + min_x_distance),
                    std::min(max_y, pt.y + min_y_distance));
    ...
    probability_object->Set(Nan::New<String>("probability").ToLocalChecked(),
                            probability_value);
...
NAN_METHOD(Matrix::MatchTemplate) {
  ...
  int method =
      (info.Length() < 2) ? (int)cv::TM_CCORR_NORMED : info[1]->Uint32Value();
...
NAN_METHOD(Matrix::PutText) {
  ...
  Matrix *self = Nan::ObjectWrap::Unwrap<Matrix>(info.This());
  Nan::Utf8String textString(info[0]); // FIXME: might cause issues, see here
                                       // https://github.com/rvagg/nan/pull/152
  ...
  if (!strcmp(font, "HERSEY_SIMPLEX")) {
    constFont = cv::FONT_HERSHEY_SIMPLEX;
  } else if (!strcmp(font, "HERSEY_PLAIN")) {
    constFont = cv::FONT_HERSHEY_PLAIN;
  } else if (!strcmp(font, "HERSEY_DUPLEX")) {
    constFont = cv::FONT_HERSHEY_DUPLEX;
  } else if (!strcmp(font, "HERSEY_COMPLEX")) {
    constFont = cv::FONT_HERSHEY_COMPLEX;
  } else if (!strcmp(font, "HERSEY_TRIPLEX")) {
    constFont = cv::FONT_HERSHEY_TRIPLEX;
  } else if (!strcmp(font, "HERSEY_COMPLEX_SMALL")) {
    constFont = cv::FONT_HERSHEY_COMPLEX_SMALL;
  } else if (!strcmp(font, "HERSEY_SCRIPT_SIMPLEX")) {
    constFont = cv::FONT_HERSHEY_SCRIPT_SIMPLEX;
  } else if (!strcmp(font, "HERSEY_SCRIPT_COMPLEX")) {
    constFont = cv::FONT_HERSHEY_SCRIPT_COMPLEX;
  } else if (!strcmp(font, "HERSEY_SCRIPT_SIMPLEX")) {
    constFont = cv::FONT_HERSHEY_SCRIPT_SIMPLEX;
  }
  ...
  cv::putText(self->mat, text, cv::Point(x, y), constFont, scale, color,
              thickness);
...
NAN_METHOD(Matrix::GetPerspectiveTransform) {
  ...
  for (unsigned int i = 0; i < 4; i++) {
    src_corners[i] = cvPoint(srcArray->Get(i * 2)->IntegerValue(),
                             srcArray->Get(i * 2 + 1)->IntegerValue());
    tgt_corners[i] = cvPoint(tgtArray->Get(i * 2)->IntegerValue(),
                             tgtArray->Get(i * 2 + 1)->IntegerValue());
  }
...
NAN_METHOD(Matrix::Shift) {
  ...
  cv::Rect roi =
      cv::Rect(std::max(-deltai.x, 0), std::max(-deltai.y, 0), 0, 0) +
      self->mat.size();
  res = padded(roi);
  ~self->mat;
  self->mat = res;
