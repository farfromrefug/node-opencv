...
Matrix::Matrix() : node_opencv::Matrix()
{
  mat = cv::Mat();
}
...
{
  if ((type != cv::NORM_MINMAX) && (type != cv::NORM_INF) && (type != cv::NORM_L1) && (type != cv::NORM_L2) && (type != cv::NORM_L2SQR) && (type != cv::NORM_HAMMING) && (type != cv::NORM_HAMMING2) && (type != cv::NORM_RELATIVE) && (type != cv::NORM_TYPE_MASK))
  {
...
public:
  AsyncToBufferWorker(Nan::Callback *callback, Matrix *matrix, std::string ext,
                      std::vector<int> params) : Nan::AsyncWorker(callback),
                                                 matrix(matrix),
                                                 ext(ext),
                                                 params(params)
  {
...
{
public:
  AsyncSaveWorker(Nan::Callback *callback, Matrix *matrix, char *filename) : Nan::AsyncWorker(callback),
                                                                             matrix(matrix),
                                                                             filename(filename)
  {
  }
...
NAN_METHOD(Matrix::Rotate)
{
  ...
  // we do a fast thing
  bool rightOrStraight = (ceil(angle) == angle) && (!((int)angle % 90)) && (info.Length() == 1);
  if (rightOrStraight)
  {
